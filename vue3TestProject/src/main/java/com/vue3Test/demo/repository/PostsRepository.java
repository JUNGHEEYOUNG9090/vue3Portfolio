...
public interface PostsRepository extends JpaRepository<PostsModel, Long> {
	...
	@Query(value = "SELECT id, title, update_dt, type "
			+ "FROM ("
			+ "    SELECT d.id, d.title, d.update_dt, 'devLog' AS type "
			+ "    FROM dev_log d "
			+ "    UNION ALL "
			+ "    SELECT image_id as id, i.image_src AS title, i.update_dt, 'image' AS type "
			+ "    FROM imagelistinfo i"
			+ ") as union_table "
			+ "ORDER BY update_dt DESC, id DESC ",
